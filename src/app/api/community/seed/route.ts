 ...
 * Seed community with demo posts
  ...
  try {
    ...
    for (const postData of postsData) {
      ...
      if (existingPost) {
        ...
        createdPosts.push({ post: existingPost, skipped: true });
        continue;
      }
      ...
      createdPosts.push({ post, skipped: false });
    ...
    return NextResponse.json(
      { error: "Failed to seed community", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
